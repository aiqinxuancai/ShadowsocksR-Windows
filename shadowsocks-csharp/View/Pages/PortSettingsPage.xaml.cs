...
{
    public partial class PortSettingsPage : Page
    {
        ...
        public PortSettingsPage()
        {
            InitializeComponent();
            ApplicationThemeManager.Apply(this);
        }

        public PortSettingsPage(MainController controller)
        {
        ...
        private void Page_Loaded(object sender, RoutedEventArgs _)
        {
            ...
            ValueChanged += PortSettingsPage_ValueChanged;

            foreach (var c in ViewUtils.FindVisualChildren<TextBox>(this))
            {
                if (c.Name.EndsWith(@"TextBox"))
                {
                    c.TextChanged += (o, e) => ValueChanged?.Invoke(this, EventArgs.Empty);
                }
            }
            foreach (var c in ViewUtils.FindVisualChildren<CheckBox>(this))
            {
                c.Checked += (o, e) => ValueChanged?.Invoke(this, EventArgs.Empty);
                c.Unchecked += (o, e) => ValueChanged?.Invoke(this, EventArgs.Empty);
            }
            foreach (var c in ViewUtils.FindVisualChildren<ComboBox>(this))
            {
                c.SelectionChanged += (o, e) => ValueChanged?.Invoke(this, EventArgs.Empty);
            }
            foreach (var c in ViewUtils.FindVisualChildren<NumberUpDown>(this))
            {
                c.ValueChanged += (o, e) => ValueChanged?.Invoke(this, EventArgs.Empty);
            }

            LoadCurrentConfiguration();
        }

        private void PortSettingsPage_ValueChanged(object sender, EventArgs e)
        {
            // Value changed
        }
        ...
        private void LoadItems()
        {
            var items = new[]
            {
                new {Text = Resources[@"PortForward"] as string, Value = PortMapType.Forward},
                new {Text = Resources[@"ForceProxy"] as string, Value = PortMapType.ForceProxy},
                new {Text = Resources[@"ProxyWithRule"] as string, Value = PortMapType.RuleProxy}
            };
        ...
        private void AddButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                _modifiedConfiguration.PortMap[key] = new PortMapConfig();
                PortSettingsPage_ValueChanged(this, EventArgs.Empty);
            }
